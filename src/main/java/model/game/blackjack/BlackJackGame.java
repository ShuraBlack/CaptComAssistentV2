...
public class BlackJackGame {
    ...
    private void createTransaction(BJPlayer player) {
        ConnectionUtil.executeSQL(ConnectionUtil.UPDATE_GAME_BLACKJACK(player.getUserid()
    ...
    public EmbedBuilder reset() {
        ...
        player.removeIf(pl -> {
            if (pl.getMoney() < (pl.getBet() * 100)) {
                createTransaction(pl);
                s.append(DiscordBot.INSTANCE.getManager().retrieveUserById(pl.getUserid()).complete().getAsMention()).append("\n");
                return true;
            }
            return false;
        });
