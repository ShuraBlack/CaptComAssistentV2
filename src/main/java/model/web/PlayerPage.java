    ...
    public void handle(HttpExchange he) throws IOException {
        if (he.getRequestMethod().equals("POST")) {
            ...
            if (result.toString().contains("stop")) {
                ...
                DiscordBot.INSTANCE.getPlayerManager().loadItemOrdered(musicManager, args, new LoadWebPlayerHandler(musicManager
                        , ServerUtil.getTextChannel(ServerUtil.getChannelID("music"))));
    ...
    public String fillContent(String page) {
        ...
        if (!trackScheduler.getQueue().isEmpty()) {
            ...
            for (AudioTrack audioTrack : trackScheduler.getQueue()) {
                if (i == 7) {
                    s.append("<li>" + (trackScheduler.getQueue().size()-i) + " weitere Lieder</li>");
                    break;
                }
